...
import { ChangeFormat, setFormat } from "../store/weather";
const Navbar = () => {
  const { chosenFormat, current } = useSelector((store) => store.weather);
  const [isOpen, setIsOpen] = useState(false);
  const dispatch = useDispatch();
  const handleClick = () => {
    if (current === null) {
      if (chosenFormat === "metric")
        dispatch(setFormat({ setting: "imperial" }));
      else dispatch(setFormat({ setting: "metric" }));
    } else {
  ...
  return (
    <nav className="relative flex gap-2 flex-between mb-14">
      <div className="w-3xs">
        <svg
      ...
      <button
        ...
        className="gap-2 btn-neutral flex-center"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
      ...
      {isOpen && (
        <div className="absolute z-100 p-2 py-4 text-Neutral-200 min-w-[25%]  right-0 top-[130%] rounded-2xl bg-Neutral-800">
          <button
            className="min-w-full p-4 text-left rounded-2xl bg-Neutral-700 hover:bg-Neutral-600"
            onClick={handleClick}
          >
            Switch to {chosenFormat === "metric" ? "Imperial" : "Metric"}
          </button>
          <div>
            <p className="p-2 text-Neutral-300">Temperature</p>
            <div
              className={`min-w-full p-4 flex-between text-left rounded-2xl  ${
                chosenFormat === "metric" && "bg-Neutral-700"
              }`}
            >
              <span>Celsius (°C)</span>
              {chosenFormat === "metric" && (
            ...
            <div
              className={`min-w-full p-4 flex-between text-left rounded-2xl  ${
                chosenFormat === "imperial" && "bg-Neutral-700"
              }`}
            >
              <span>Fahrenheit (°F)</span>
              {chosenFormat === "imperial" && (
                <span>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="14"
          ...
          <div>
            <p className="p-2 text-Neutral-300">Wind Speed</p>
            <div
              className={`min-w-full p-4 flex-between text-left rounded-2xl  ${
                chosenFormat === "metric" && "bg-Neutral-700"
              }`}
          ...
          <div>
            <p className="p-2 text-Neutral-300">Precipitation</p>
            <div
              className={`min-w-full p-4 flex-between text-left rounded-2xl  ${
                chosenFormat === "metric" && "bg-Neutral-700"
              }`}
